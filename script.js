...
// Event Listeners for Drag & Drop and File Input
...
uploadSection.addEventListener('drop', e => {
    ...
    if (files.length && files[0].type === 'application/pdf') handleFileSelect({
        target: {
            files
        }
    });
...
function handleFileSelect(event) {
    ...
    if (typeof gtag !== 'undefined') gtag('event', 'file_upload', {
        'event_category': 'conversion',
        'event_label': 'pdf_uploaded'
    });
}
...
// Apply Scanned Effect to Canvas
...
function setupDownload() {
    ...
    btn.onclick = () => {
        const blob = new Blob([convertedPdfBytes], {
